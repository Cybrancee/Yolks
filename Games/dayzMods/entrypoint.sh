...
function RunSteamCMD { #[Input: int server=0 mod=1; int id]
    ...
    while (( $updateAttempt < $STEAMCMD_ATTEMPTS )); do # Loop for specified number of attempts
        ...
        if [[ $1 == 0 ]]; then # Server
            ...
            ${DEPOTDOWNLOADER_DIR} -app "${GAME_ID}" -pubfile "$2" -username "${STEAM_USER}" -password "${STEAM_PASS}" -dir "/home/container/@$2" 2>&1 | tee -a "${STEAMCMD_LOG}"
        fi
...
mkdir -p /home/container/depotdownloader

# Fetch the download URL for the latest AMD64 release and download it
curl -sL $(curl -s https://api.github.com/repos/SteamRE/DepotDownloader/releases/latest | grep "browser_download_url" | grep "linux-x64.zip" | cut -d '"' -f 4) -o /tmp/depotdownloader.zip

# Extract the files and overwrite any older version
unzip -q -o /tmp/depotdownloader.zip -d /home/container/depotdownloader

# Ensure the main binary is executable
chmod +x /home/container/depotdownloader/DepotDownloader

DEPOTDOWNLOADER_DIR="/home/container/depotdownloader/DepotDownloader"
...
if [[ ${UPDATE_SERVER} == 1 ]]; then
    ...
    echo -e "\n${GREEN}[UPDATE]:${NC} Verifying and updating ${CYAN}Steam Workshop mods${NC} via SteamCMD..."
    
    # Delete SteamCMD appworkshop cache once before the loop to prevent download failures
    if [[ -f "${WORKSHOP_DIR}/appworkshop_$GAME_ID.acf" ]]; then
    ...
    do
        if [[ $modID =~ ^[0-9]+$ ]]; then # Only process valid mod IDs
            echo -e "\n${GREEN}[UPDATE]:${NC} Processing Mod ID: ${CYAN}${modID}${NC}..."
            
            # Let SteamCMD handle the check. It will skip downloading if already up to date.
            RunSteamCMD 1 "$modID"
        fi
    done
    echo -e "\n${GREEN}[UPDATE]:${NC} Steam Workshop mod update check ${GREEN}complete${NC}!"
fi
fi
